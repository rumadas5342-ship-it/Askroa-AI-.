...
class ElevenLabsTtsService {
  // API key removed from client — all TTS calls go through backend /api/voice/tts
  // This prevents API key exposure in APK/IPA binaries.
  ...
  // Primary: just_audio plays ElevenLabs MP3 from backend
  final AudioPlayer _player = AudioPlayer();
  // Fallback: flutter_tts used when backend is unreachable or returns error
  ...
  /// Initialise flutter_tts once so it is ready for instant fallback
  Future<void> _initFlutterTts() async {
    try {
      ...
      if (Platform.isAndroid) {
        await _flutterTts.setQueueMode(1); // flush previous utterance
      }
      _flutterTtsReady = true;
      _log.d('[TTS] flutter_tts ready (fallback engine)');
    } catch (e) {
      _log.w('[TTS] flutter_tts init failed — fallback unavailable', error: e);
    }
  }
  ...
  /// Primary: calls backend /api/voice/tts (ElevenLabs MP3).
  /// Fallback: flutter_tts (device TTS) when backend fails — user never hears silence.
  Future<void> speak(String text, {
    ...
    if (backendUrl.isNotEmpty && accessToken != null && accessToken.isNotEmpty) {
      try {
        final request = http.MultipartRequest(
          'POST',
          Uri.parse('$backendUrl/api/voice/tts'),
        )
          ..headers['Accept']        = 'audio/mpeg'
          ..headers['Authorization'] = 'Bearer $accessToken'
          ..fields['text']      = text.length > 5000 ? text.substring(0, 5000) : text
          ..fields['voice_key'] = voiceKey;

        final streamed = await request.send()
            .timeout(const Duration(seconds: 30)); // tighter timeout so fallback fires fast
        ...
        if (streamed.statusCode == 200) {
          ...
          await _player.play();
          // Clean up temp file + fire onDone after playback
          ...
          _log.d('[TTS] ElevenLabs via backend: voice=$voiceKey (${text.length} chars)');
        } else {
          _log.w('[TTS] Backend returned ${streamed.statusCode} — falling back to flutter_tts');
        }
      } catch (e) {
        _log.w('[TTS] Backend unreachable — falling back to flutter_tts', error: e);
      }
    }

    // ── Fallback: flutter_tts (device TTS engine) ────────────────────────────
    if (!backendSucceeded) {
      await _speakWithFlutterTts(text, voiceKey: voiceKey, onDone: onDone);
    }
  }

  /// Voice personality map for flutter_tts fallback.
  /// ElevenLabs voice names are mapped to pitch/rate tuning so the
  /// device engine approximates the same character.
  static const Map<String, _FallbackVoiceProfile> _fallbackProfiles = {
    'voice_1': _FallbackVoiceProfile(name: 'Rachel', gender: 'female', pitch: 1.1, rate: 0.48, lang: 'en-US'),
    'voice_2': _FallbackVoiceProfile(name: 'Antoni', gender: 'male',   pitch: 0.85, rate: 0.50, lang: 'en-US'),
    'voice_3': _FallbackVoiceProfile(name: 'Elli',   gender: 'female', pitch: 1.2, rate: 0.52, lang: 'en-US'),
    'voice_4': _FallbackVoiceProfile(name: 'Simran', gender: 'female', pitch: 1.05, rate: 0.48, lang: 'en-IN'),
    'voice_5': _FallbackVoiceProfile(name: 'Harry',  gender: 'male',   pitch: 0.80, rate: 0.46, lang: 'en-GB'),
  };

  /// Fallback TTS using the device engine (flutter_tts).
  /// Applies per-voice pitch/rate profile to approximate the ElevenLabs character.
  /// Works completely offline — no network required.
  Future<void> _speakWithFlutterTts(
    String text, {
    String voiceKey = 'voice_1',
    VoidCallback? onDone,
  }) async {
    try {
      if (!_flutterTtsReady) await _initFlutterTts();
      final profile = _fallbackProfiles[voiceKey] ?? _fallbackProfiles['voice_1']!;
      final truncated = text.length > 3000 ? text.substring(0, 3000) : text;

      // Apply voice character profile
      await _flutterTts.setLanguage(profile.lang);
      await _flutterTts.setPitch(profile.pitch);
      await _flutterTts.setSpeechRate(profile.rate);

      // Try to pick a matching platform voice by gender
      try {
          final voices = await _flutterTts.getVoices as List?;
          if (voices != null && voices.isNotEmpty) {
            // Find a voice matching the gender in the right locale
            final matched = voices.cast<Map>().where((v) {
              final name   = (v['name']   as String? ?? '').toLowerCase();
              final locale = (v['locale'] as String? ?? '').toLowerCase();
              final genderMatch = profile.gender == 'female'
                  ? (name.contains('female') || name.contains('woman') || name.contains('girl') ||
                     name.contains('samantha') || name.contains('victoria') || name.contains('karen') ||
                     name.contains('moira') || name.contains('tessa') || name.contains('veena') ||
                     name.contains('fiona') || name.contains('allison') || name.contains('susan') ||
                     name.contains('zira') || name.contains('hazel'))
                  : (name.contains('male') || name.contains('man') || name.contains('guy') ||
                     name.contains('daniel') || name.contains('alex') || name.contains('fred') ||
                     name.contains('thomas') || name.contains('rishi') || name.contains('david') ||
                     name.contains('mark') || name.contains('george'));
              final localeMatch = locale.startsWith(profile.lang.substring(0, 2).toLowerCase());
              return genderMatch && localeMatch;
            }).toList();
            if (matched.isNotEmpty) {
              await _flutterTts.setVoice({
                'name':   matched.first['name'] as String,
                'locale': matched.first['locale'] as String,
              });
            }
          }
      ...
      await _flutterTts.speak(truncated);
      _log.d('[TTS] flutter_tts fallback: voice=$voiceKey (${profile.name}/${profile.gender}) '
             'pitch=${profile.pitch} rate=${profile.rate} (${truncated.length} chars)');
    } catch (e, st) {
      _log.e('[TTS] flutter_tts fallback failed', error: e, stackTrace: st);
    }
  }
...
/// Fallback TTS profile — maps ElevenLabs voice character to device TTS params
class _FallbackVoiceProfile {
  final String name;
  final String gender;
  final double pitch;
  final double rate;
  final String lang;
  const _FallbackVoiceProfile({
    required this.name,
...
class VoiceSettingsNotifier extends StateNotifier<VoiceSettings> {
  ...
  Future<void> _init() async {
    try {
      ...
      // Load from local storage (SharedPrefs → SecureStorage fallback)
  ...
  // ── Storage keys ──────────────────────────────────────────────────────────
  static const String _prefKey       = 'askroa_voice_settings';      // SharedPreferences
  static const String _secureKey     = 'askroa_voice_settings_sec';  // FlutterSecureStorage
  // Voice is saved in TWO places:
  //   1. SharedPreferences  — fast local read on startup (no async unlock needed)
  //   2. FlutterSecureStorage — survives app-data clear/reinstall on Android,
  //                             and iCloud Keychain sync on iOS between devices
  //   3. Backend DB          — source-of-truth, loaded on login

  Future<void> setVoice(String voiceKey) async {
    final next = state.copyWith(voiceKey: voiceKey);
    state = next;
    // Save locally first so UI is instant
    await _persist(next);
    // Then push to backend (non-blocking on failure)
    await _syncToBackend(next);
    _log.d('[VoiceSettings] Voice set: $voiceKey');
  }
  ...
  /// Save to SharedPreferences (fast) + FlutterSecureStorage (persistent)
  Future<void> _persist(VoiceSettings s) async {
    final json = jsonEncode(s.toJson());
    // 1. SharedPreferences — fast, survives normal app restarts
    try {
      final p = await SharedPreferences.getInstance();
      await p.setString(_prefKey, json);
    } catch (e) {
      _log.w('[VoiceSettings] SharedPreferences save failed', error: e);
    }
    // 2. FlutterSecureStorage — persistent across reinstall / device migration
    try {
      await AppSecureStorage.instance.write(key: _secureKey, value: json);
    } catch (e) {
      _log.w('[VoiceSettings] SecureStorage save failed (non-critical)', error: e);
    }
  }

  /// On init: try SharedPreferences first, fall back to SecureStorage
  Future<VoiceSettings?> _loadLocal() async {
    // Try SharedPreferences first (fastest)
    try {
      final p   = await SharedPreferences.getInstance();
      final raw = p.getString(_prefKey);
      if (raw != null) return VoiceSettings.fromJson(jsonDecode(raw) as Map<String, dynamic>);
    } catch (_) {}
    // Fallback: SecureStorage (survives reinstall)
    try {
      final raw = await AppSecureStorage.instance.read(key: _secureKey);
      if (raw != null) return VoiceSettings.fromJson(jsonDecode(raw) as Map<String, dynamic>);
    } catch (_) {}
    return null;
  }
  ...
  Future<void> _syncToBackend(VoiceSettings s) async {
    try {
      ...
      _log.w('[VoiceSettings] backend sync failed (will retry on next setVoice)', error: e);
    }
  }

  /// Called on login: load from backend (source of truth) and persist locally
  Future<void> loadFromBackend() async {
    try {
      ...
      if (data['voice_key'] != null) {
        ...
        state = loaded;
        await _persist(loaded);   // write to both local stores
        _log.d('[VoiceSettings] Loaded from backend: ${loaded.voiceKey}');
      }
    } catch (e) {
      _log.w('[VoiceSettings] backend load failed — keeping local settings', error: e);
    }
  }
}
  ...
  static Future<T> load<T>(String key, Future<T> Function() loader) {
    return _lock.synchronized(() async {
      ...
      if (!_loadingFutures.containsKey(key)) {
        // Store the future so concurrent callers await the same work
      ...
      // Await the in-flight future — returns T, not Future<T>
      return await (_loadingFutures[key] as Future<T>);
    });
  }
...
const String appVersion = '3.4.0'; // v42: TTS voice profiles + dual-storage
...
class WebSocketService {
  ...
  Future<void> connect() async {
    ...
    try {
      ...
      // Attach JWT token as query param — backend verifies before accepting
      String? wsToken;
      try {
        ...
        if (encryptedToken != null) {
          ...
          wsToken = await encryption.decrypt(encryptedToken);
        }
      } catch (e) {
        _logger.w('[WS] Could not read token for connection', error: e);
      }

      Uri uri;
      if (wsToken != null && wsToken.isNotEmpty) {
        final base = Uri.parse(wsUrl);
        uri = base.replace(queryParameters: {...base.queryParameters, 'token': wsToken});
      } else {
        uri = Uri.parse(wsUrl);
      }
...
class SecurityArchitecture {
  ...
  Future<String?> getAccessToken() async {
    ...
    try {
      final encryptedToken = await _secureStorage.read(key: 'askroa_access_token');
      final expiryString = await _secureStorage.read(key: 'askroa_token_expiry');
  ...
  Future<String?> refreshAccessToken() async {
    try {
      final encryptedRefreshToken = await _secureStorage.read(key: 'askroa_refresh_token');
  ...
  Future<void> clearTokens() async {
    ...
    await _secureStorage.delete(key: 'askroa_token_expiry');
...
// ===========================================
// DEVICE TIER DETECTION — LOW-END PHONE SUPPORT
// ===========================================
...
class DeviceTierDetector {
  ...
  static Future<DeviceTier> detect() async {
    ...
    try {
      ...
      if (Platform.isAndroid) {
        ...
        final ram = ProcessInfo.maxRss; // bytes — rough upper bound
        final ramMb = ram ~/ (1024 * 1024);
        // SDK < 26 = Android 7 era, SDK < 29 = Android 9 era
        if (sdk < 26 || ramMb < 1500) {
          _cachedTier = DeviceTier.low;
        } else if (sdk < 31 || ramMb < 3000) {
        ...
        // iPhone 8/X era and older = low, 11-13 = mid, 14+ = high
  ...
  static DeviceTier get current => _cachedTier ?? DeviceTier.mid;
  static bool get isLow => current == DeviceTier.low;
  static bool get isMidOrLow => current != DeviceTier.high;
}

// ===========================================
// ADAPTIVE PERFORMANCE MANAGER
// Adjusts image cache, animation, polling, frame target
// based on device tier to prevent lag/heat/battery drain
// ===========================================
class AdaptivePerformanceManager {
  static bool _initialized = false;
  static Timer? _memoryPressureTimer;

  static Future<void> initialize() async {
    if (_initialized) return;
    _initialized = true;

    final tier = await DeviceTierDetector.detect();
    _applyImageCacheLimits(tier);
    _applyFrameSchedulerHints(tier);
    _startMemoryPressureWatcher(tier);

    if (Platform.isAndroid) {
      _applyAndroidWindowFlags();
    }
  }

  /// Image cache: low-end gets 30MB / 50 images, high-end gets 150MB / 1000
  static void _applyImageCacheLimits(DeviceTier tier) {
    switch (tier) {
      case DeviceTier.low:
  ...
  /// On low-end: hint Flutter to target 30fps for non-interaction phases
  static void _applyFrameSchedulerHints(DeviceTier tier) {
    if (tier == DeviceTier.low) {
      // Reduce idle frame scheduling pressure
      SchedulerBinding.instance.schedulerPhase;
      // Lower engine frame rate hint via system channel
  ...
  /// Watch RSS every 20s on low-end, 60s on high-end.
  /// If too high → trim image cache immediately.
  static void _startMemoryPressureWatcher(DeviceTier tier) {
    final interval = tier == DeviceTier.low
        ? const Duration(seconds: 20)
        : tier == DeviceTier.mid
            ? const Duration(seconds: 40)
            : const Duration(seconds: 60);

    final warnMb = tier == DeviceTier.low ? 180 : tier == DeviceTier.mid ? 280 : 400;

    _memoryPressureTimer = Timer.periodic(interval, (_) {
      try {
  ...
  /// Hardware-accelerated rendering flags for Android
  static void _applyAndroidWindowFlags() {
    SystemChrome.setSystemUIOverlayStyle(const SystemUiOverlayStyle(
      statusBarColor: Colors.transparent,
    ));
  }

  /// Call this when app goes to background — frees memory immediately
  static void onBackground() {
    if (DeviceTierDetector.isMidOrLow) {
      PaintingBinding.instance.imageCache.clear();
    }
  }

  /// Polling interval for background tasks — longer on low-end = less battery drain
  static Duration get backgroundSyncInterval {
    switch (DeviceTierDetector.current) {
      case DeviceTier.low:  return const Duration(minutes: 30);
      case DeviceTier.mid:  return const Duration(minutes: 15);
      case DeviceTier.high: return const Duration(minutes: 10);
    }
  }

  /// Debounce duration — low-end gets longer debounce to skip frames
  static Duration get inputDebounce {
    return DeviceTierDetector.isLow
        ? const Duration(milliseconds: 500)
        : const Duration(milliseconds: 300);
  }

  /// Whether to show heavy animations (lottie, shimmer) on this device
  static bool get shouldShowHeavyAnimations => !DeviceTierDetector.isLow;

  /// Max concurrent image loads — reduce on low-end to avoid OOM
  static int get maxConcurrentImageLoads {
    switch (DeviceTierDetector.current) {
      case DeviceTier.low:  return 2;
      case DeviceTier.mid:  return 4;
      case DeviceTier.high: return 8;
    }
  }
...
class PerformanceOptimizer {
  ...
  // Low-end targets 30fps to reduce GPU/CPU heat; high-end stays at 60fps
  static int get _targetFps => DeviceTierDetector.isLow ? 30 : 60;
  static int get _frameTimeTarget => DeviceTierDetector.isLow ? 33 : 16;
  ...
  // Thresholds adapt to device tier — low-end devices warn earlier
  static int get _memoryWarningThreshold {
    switch (DeviceTierDetector.current) {
      case DeviceTier.low:  return 120;
      case DeviceTier.mid:  return 200;
      case DeviceTier.high: return 350;
    }
  }
  static int get _memoryCriticalThreshold {
    switch (DeviceTierDetector.current) {
      case DeviceTier.low:  return 180;
      case DeviceTier.mid:  return 300;
      case DeviceTier.high: return 500;
    }
  }
  ...
  void _startMetricsCollection() {
    // Low-end: collect every 10s to reduce CPU wake-ups; high-end: every 5s
    final interval = DeviceTierDetector.isLow
        ? const Duration(seconds: 10)
        : const Duration(seconds: 5);
    _metricsTimer = Timer.periodic(interval, (timer) {
      if (!_isDisposed) {
  ...
  void _collectMemoryMetrics() {
    try {
      ...
      if (rssMb > _memoryCriticalThreshold) {
        _logger.w('[Perf] Critical memory: ${rssMb}MB — clearing image cache');
        PaintingBinding.instance.imageCache.clear();
        PaintingBinding.instance.imageCache.clearLiveImages();
      } else if (rssMb > _memoryWarningThreshold) {
  ...
  void debounce(VoidCallback action, {Duration? duration}) {
    final effectiveDuration = duration ?? AdaptivePerformanceManager.inputDebounce;
    _debounceTimer?.cancel();
    _debounceTimer = Timer(effectiveDuration, action);
  }
...
class AskroaModelInfo {
  ...
  AskroaModelInfo withRequiredPlan(String? plan) => AskroaModelInfo(
    id: id,
  ...
  String get planLabel {
    final p = requiredPlan ?? tier.backendPlanKey;
    switch (p.toLowerCase().trim()) {
      case 'free':      return '';
      case 'monthly':   return 'Pro';
      case 'half_year': return 'Pro+';
      case 'yearly':    return 'Ultra Pro';
      default:          return '';
    }
  }

  Color get planLabelColor {
    final p = requiredPlan ?? tier.backendPlanKey;
    switch (p.toLowerCase().trim()) {
      case 'monthly':   return const Color(0xFF007AFF);
      case 'half_year': return const Color(0xFFFFD700);
      case 'yearly':    return const Color(0xFFFF6B35);
...
// ===========================================
// BACKEND-DRIVEN MODEL LIST WITH PLAN LABELS
// ===========================================

@immutable
class ModelsByPlanState {
  final List<AskroaModelInfo> models;
  final bool isLoading;
  final String? error;
  const ModelsByPlanState({
    this.models = const [],
    this.isLoading = false,
    this.error,
  });
  ModelsByPlanState copyWith({
    List<AskroaModelInfo>? models,
    bool? isLoading,
    String? error,
  }) => ModelsByPlanState(
...
class ModelsByPlanNotifier extends StateNotifier<ModelsByPlanState> {
  final Ref _ref;
  ProductionLogger? _logger;

  ModelsByPlanNotifier(this._ref) : super(const ModelsByPlanState()) {
    _logger = _ref.read(loggerProvider);
    _fetch();
  }
  ...
  Future<void> _fetch() async {
    ...
    try {
      final api  = await _ref.read(apiServiceProvider.future);
      final data = await api.fetchModelsByPlan();
      final merged = _merge(data);
      if (mounted) state = state.copyWith(models: merged, isLoading: false);
      _logger?.i('[ModelsByPlan] Fetched ${merged.length} models with plan info');
    } catch (e) {
      _logger?.w('[ModelsByPlan] Fetch failed, falling back to static list', error: e);
      if (mounted) {
        state = state.copyWith(
          models: kAskroaModels,
          isLoading: false,
          error: e.toString(),
        );
      }
  ...
  String _normalizePlan(String raw) {
    switch (raw.toLowerCase().trim()) {
      ...
      case 'half_year':
      case 'pro':
      case 'pro+':      return 'half_year';
      case 'yearly':
      case 'elite':
      case 'ultra_pro':
      case 'ultra pro': return 'yearly';
      default:          return raw.toLowerCase().trim();
    }
  }

  List<AskroaModelInfo> _merge(Map<String, dynamic> data) {
    final Map<String, String> planByModelId = {};

    // Backend response shape A: {"models": [{"id": "gpt-4o", "required_plan": "monthly", ...}]}
    final modelsList = data['models'] as List?;
    if (modelsList != null) {
      for (final raw in modelsList) {
        final m    = raw as Map<String, dynamic>? ?? {};
        final id   = m['id'] as String? ?? m['model_id'] as String? ?? '';
        final plan = m['required_plan'] as String?
            ?? m['plan'] as String?
            ?? m['min_plan'] as String?;
        if (id.isNotEmpty && plan != null) {
          planByModelId[id] = _normalizePlan(plan);
        }
      }
    }

    // Backend response shape B: {"free": [...], "monthly": [...], "half_year": [...], "yearly": [...]}
    const knownKeys = {'free', 'monthly', 'basic', 'half_year', 'pro', 'yearly', 'elite'};
    for (final entry in data.entries) {
      if (!knownKeys.contains(entry.key.toLowerCase())) continue;
      final ids = entry.value as List?;
      if (ids == null) continue;
      final normalizedPlan = _normalizePlan(entry.key);
      for (final id in ids) {
        final idStr = id as String? ?? '';
        if (idStr.isNotEmpty) planByModelId.putIfAbsent(idStr, () => normalizedPlan);
      }
    }

    return kAskroaModels.map((m) {
      final backendPlan = planByModelId[m.id];
      return backendPlan != null ? m.withRequiredPlan(backendPlan) : m;
    }).toList();
  }
...
final modelsByPlanProvider =
    StateNotifierProvider<ModelsByPlanNotifier, ModelsByPlanState>(
  (ref) => ModelsByPlanNotifier(ref),
);

// Returns the plan label string for a given model id (e.g. "Ultra Pro only")
// Use in UI: ref.watch(modelPlanLabelProvider(modelId))
final modelPlanLabelProvider = Provider.family<String, String>((ref, modelId) {
  final models = ref.watch(modelsByPlanProvider).models;
  try {
    final m = models.firstWhere((m) => m.id == modelId);
    return m.planLabel;
  } catch (_) {
    return '';
  }
...
// Returns Color for the plan label badge
final modelPlanLabelColorProvider = Provider.family<Color, String>((ref, modelId) {
  final models = ref.watch(modelsByPlanProvider).models;
  try {
    final m = models.firstWhere((m) => m.id == modelId);
    return m.planLabelColor;
  } catch (_) {
    ...
    return MigrationStrategy(
      ...
      onUpgrade: (Migrator m, int from, int to) async {
        try {
          ...
          // PRODUCTION SAFE: Log but do NOT rethrow.
          // Rethrowing here crashes the app and destroys user data.
          // Missing columns will use Drift defaults. A future schemaVersion
          // bump will retry any failed steps. Sentry captures this.
          debugPrint('[DB Migration] onUpgrade $from→$to failed: $e\n$stack');
          // Intentionally not rethrowing — user data preservation is priority.
        }
      },
    );
  }
...
class ProductionApiService {
  ...
  Future<Map<String, dynamic>> _getDeviceInfo() async {
    ...
    return {
      ...
      'device_model': Platform.isAndroid ? 'Android' : (Platform.isIOS ? 'iOS' : Platform.operatingSystem),
...
class InAppPurchaseService {
  ...
  Future<void> _verifyCreditPackPurchase(PurchaseDetails purchase) async {
    try {
      ...
      final envConfig = EnvironmentConfig();
      // FastAPI Form() — must be multipart/form-data, NOT JSON
      final _creditReq = http.MultipartRequest(
        'POST',
        Uri.parse('${envConfig.backendBaseUrl}/api/credits/purchase'),
      )
        ..headers['Authorization'] = 'Bearer $token'
        ..fields['pack_id']        = purchase.productID
        ..fields['purchase_token'] = purchase.verificationData.serverVerificationData
        ..fields['platform']       = Platform.operatingSystem
        ..fields['idempotency_key'] = '${purchase.productID}_${purchase.purchaseID ?? DateTime.now().millisecondsSinceEpoch}';
      final _creditStreamed = await _creditReq.send().timeout(const Duration(seconds: 15));

      if (_creditStreamed.statusCode == 200) {
        _logger.i('[IAP] Credit pack verified: ${purchase.productID}');
      } else {
        _logger.w('[IAP] Credit pack verification failed ${_creditStreamed.statusCode}');
      }
    } catch (e) {
  ...
  final secureStorage = AppSecureStorage.instance;
    try {
      ...
      while (attempt < maxAttempts) {
        attempt++;
        try {
          // FastAPI Form() — multipart/form-data required, NOT JSON
          // /api/ prefix added — was missing before
          final _verReq = http.MultipartRequest(
            'POST',
            Uri.parse('${envConfig.backendBaseUrl}/api/subscription/verify-purchase'),
          )
            ..headers['Authorization']   = 'Bearer $token'
            ..headers['Idempotency-Key'] = purchaseData['idempotency_key'] as String
            ..fields['product_id']               = purchaseData['product_id'] as String
            ..fields['purchase_id']              = purchaseData['purchase_id'] as String
            ..fields['server_verification_data'] = purchaseData['server_verification_data'] as String
            ..fields['platform']                 = purchaseData['platform'] as String
            ..fields['idempotency_key']          = purchaseData['idempotency_key'] as String;
          final _verStreamed = await _verReq.send().timeout(const Duration(seconds: 15));
          final response     = await http.Response.fromStream(_verStreamed);
  ...
  Future<bool> _confirmDeliveryWithServer(String token, PurchaseDetails purchase) async {
    try {
      ...
      // FastAPI Form() — multipart/form-data required + /api/ prefix fixed
      final _cdReq = http.MultipartRequest(
        'POST',
        Uri.parse('${env.backendBaseUrl}/api/subscription/confirm-delivery'),
      )
        ..headers['Authorization'] = 'Bearer $token'
        ..fields['product_id']               = purchase.productID
        ..fields['purchase_id']              = purchase.purchaseID ?? ''
        ..fields['server_verification_data'] = purchase.verificationData.serverVerificationData
        ..fields['platform']                 = Platform.operatingSystem;
      final _cdStreamed = await _cdReq.send().timeout(const Duration(seconds: 15));
      final res = await http.Response.fromStream(_cdStreamed);
  ...
  Future<void> _syncSubscriptionWithServer(String token, String plan) async {
    try {
      final envConfig = EnvironmentConfig();
      // FastAPI Form() — multipart/form-data required + /api/ prefix fixed
      final _syncReq = http.MultipartRequest(
        'POST',
        Uri.parse('${envConfig.backendBaseUrl}/api/subscription/sync'),
      )
        ..headers['Authorization'] = 'Bearer $token'
        ..fields['plan'] = plan;
      final _syncStreamed = await _syncReq.send().timeout(const Duration(seconds: 10));
      if (_syncStreamed.statusCode == 200) {
...
final encryptionProvider = Provider<ProductionEncryptionService>((ref) {
  final logger = ref.read(loggerProvider);
  final svc = ProductionEncryptionService(logger: logger);
  ref.onDispose(svc.dispose); // FIX: cancel key-rotation / IV-cleanup timers
  return svc;
});
...
final connectivityProvider = Provider<ProductionConnectivityService>((ref) {
  final logger = ref.read(loggerProvider);
  final svc = ProductionConnectivityService(logger: logger);
  ref.onDispose(svc.dispose); // FIX: cancel StreamSubscription + health-check timer
  return svc;
});
...
final rateLimitingProvider = Provider<RateLimitingService>((ref) {
  final logger = ref.read(loggerProvider);
  final svc = RateLimitingService(logger: logger);
  ref.onDispose(svc.dispose); // FIX: cancel sync + cleanup timers
  return svc;
});
...
final webSocketProvider = Provider<WebSocketService>((ref) {
  final logger = ref.read(loggerProvider);
  final svc = WebSocketService(logger: logger);
  ref.onDispose(svc.disconnect); // FIX: cancel heartbeat/reconnect timers + close streams
  return svc;
});
...
final databaseProvider = Provider<DatabaseService>((ref) {
  final logger = ref.read(loggerProvider);
  final performance = ref.read(performanceOptimizerProvider);
  final svc = DatabaseService(logger: logger, performance: performance);
  ref.onDispose(svc.dispose); // FIX: cancel WAL checkpoint timer + close DB
  return svc;
});
...
final inAppPurchaseProvider = Provider<InAppPurchaseService>((ref) {
  final logger = ref.read(loggerProvider);
  final svc = InAppPurchaseService(logger: logger);
  ref.onDispose(svc.dispose); // FIX: cancel purchase stream sub + retry timers
  return svc;
});
...
class VoiceNotifier extends StateNotifier<VoiceState> {
  ...
  Future<void> _initialize() async {
    if ((!Platform.isAndroid && !Platform.isIOS)) {
      _logger = ref.read(loggerProvider);
      _logger.i('VoiceNotifier: voice features disabled on this platform (${Platform.operatingSystem})');
      return;
    }
    try {
      _logger = ref.read(loggerProvider);
  ...
  Future<void> speakText(String text, {String? languageCode, bool forceFlutter = false}) async {
    ...
    if (!forceFlutter && voiceSettings.useElevenLabs) {
      try {
        state = const VoiceState.speaking();
        // Read token for backend auth
        String? accessToken;
        try {
          final encToken = await AppSecureStorage.instance.read(key: 'askroa_access_token');
          if (encToken != null) {
            final enc = ProductionEncryptionService(logger: _logger);
            await enc.initialize();
            accessToken = await enc.decrypt(encToken);
          }
        } catch (_) {}
...
class _CompressParams {
  ...
  const _CompressParams({required this.bytes, required this.fileSizeBytes});
}

// Adaptive compression: quality and resolution scale with file size.
// Handles any input size without error — always returns a valid JPEG or null.
List<int>? _compressImageBytesAdaptive(_CompressParams params) {
  try {
    var image = img.decodeImage(Uint8List.fromList(params.bytes));
    if (image == null) return null;
    image = img.Image.from(image);
    try { image.exif = img.ExifData(); } catch (_) {}

    // Adaptive max dimension and quality based on file size
    final sizeMB = params.fileSizeBytes / (1024 * 1024);
    int maxDim;
    int quality;
    if (sizeMB <= 2) {
      maxDim  = 1920;
    ...
    if (image.width > image.height) {
      resized = img.copyResize(image,
          width: image.width > maxDim ? maxDim : image.width);
    } else {
      resized = img.copyResize(image,
          height: image.height > maxDim ? maxDim : image.height);
    }
...
// Legacy top-level function kept for backwards compatibility (isolate-safe)
...
class ImageUploadNotifier extends StateNotifier<ImageUploadState> {
  ...
  static const int _maxImagesPerSession = 20;
  static const int _freeUserDailyImageLimit = 1;
  static const String _freeDailyImageKey   = 'askroa_free_daily_images_count';
  static const String _freeDailyImageDate  = 'askroa_free_daily_images_date';
  ...
  // Returns how many images the current user can still upload this session.
  // Free plan: tracks daily count via SharedPreferences.
  Future<int> remainingUploadQuota({int alreadyAttachedCount = 0}) async {
    final sessionRemaining = (_maxImagesPerSession - alreadyAttachedCount).clamp(0, _maxImagesPerSession);
    final user = _userHandler.currentUser;
    if (user == null) return 0;
    if (user.plan == 'free') {
      final usedToday = await _freeDailyUsed();
      final freeRemaining = (_freeUserDailyImageLimit - usedToday).clamp(0, _freeUserDailyImageLimit);
      return freeRemaining < sessionRemaining ? freeRemaining : sessionRemaining;
    }
    return sessionRemaining;
  }

  Future<int> _freeDailyUsed() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final today = DateTime.now().toLocal();
      final storedDate = prefs.getString(_freeDailyImageDate) ?? '';
      final todayStr   = '${today.year}-${today.month}-${today.day}';
      if (storedDate != todayStr) {
        await prefs.setString(_freeDailyImageDate, todayStr);
        await prefs.setInt(_freeDailyImageKey, 0);
        return 0;
      }
      return prefs.getInt(_freeDailyImageKey) ?? 0;
    } catch (_) {
      return 0;
    }
  }

  Future<void> _incrementFreeDailyUsed(int count) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final today    = DateTime.now().toLocal();
      final todayStr = '${today.year}-${today.month}-${today.day}';
      ...
      if (storedDate != todayStr) {
        await prefs.setString(_freeDailyImageDate, todayStr);
        await prefs.setInt(_freeDailyImageKey, count);
      } else {
        final current = prefs.getInt(_freeDailyImageKey) ?? 0;
        await prefs.setInt(_freeDailyImageKey, current + count);
      }
    } catch (_) {}
  }
  ...
  Future<List<String>> uploadImages(
    ...
    return await _performance.measureOperation('upload_images', () async {
      ...
      try {
        ...
        // ── Session max 20 images guard ──────────────────────────────────────
        final sessionRemaining = _maxImagesPerSession - alreadyAttachedCount;
        if (sessionRemaining <= 0) {
          state = const ImageUploadState.error(
              'Maximum 20 images per session allowed.');
          return <String>[];
        }

        // ── Free plan daily 1 image guard ────────────────────────────────────
        List<XFile> allowed = files;
        if (user.plan == 'free') {
          final usedToday    = await _freeDailyUsed();
          final freeRemaining = (_freeUserDailyImageLimit - usedToday).clamp(0, _freeUserDailyImageLimit);
          if (freeRemaining <= 0) {
            state = const ImageUploadState.error(
                'Free plan allows 1 image upload per day. Upgrade to upload more.');
            return <String>[];
          }
          if (files.length > freeRemaining) {
            allowed = files.sublist(0, freeRemaining);
            _logger.w('[ImageUpload] Free plan: trimmed to $freeRemaining image(s)');
          }
        }

        // ── Session cap: trim if needed ──────────────────────────────────────
        if (allowed.length > sessionRemaining) {
          allowed = allowed.sublist(0, sessionRemaining);
          _logger.w('[ImageUpload] Session cap: trimmed to $sessionRemaining image(s)');
        }
        ...
        for (var i = 0; i < allowed.length; i++) {
          ...
          File file = File(allowed[i].path);

          // Robust file existence check — handle gallery / content URIs
          if (!await file.exists()) {
            try {
              final bytes = await allowed[i].readAsBytes();
              final tempDir = await getTemporaryDirectory();
              final tmp = File(
                  '${tempDir.path}/upload_src_${DateTime.now().millisecondsSinceEpoch}_$i.jpg');
              await tmp.writeAsBytes(bytes);
              file = tmp;
            } catch (e) {
              _logger.e('[ImageUpload] Cannot read file: ${allowed[i].path}', error: e);
              continue;
            }
          }
          ...
          try {
            ...
            // Always attempt compression — handles any size without error
            final compressed = await compute(_compressImageBytesAdaptive,
                _CompressParams(bytes: bytes, fileSizeBytes: fileSize));
            if (compressed != null && compressed.isNotEmpty) {
              ...
              file = compressedFile;
              _logger.d(
                  '[ImageUpload] Compressed: ${fileSize ~/ 1024}KB → ${compressed.length ~/ 1024}KB');
            }
          } catch (compressErr) {
            _logger.w('[ImageUpload] Compression failed, uploading original', error: compressErr);
          }
          ...
          _cancelTokens[allowed[i].path] = cancelToken;
          ...
          try {
            final response = await _apiService.uploadFile(
              ...
              onProgress: (progress) {
                ...
                state = ImageUploadState.uploading(
                  total: allowed.length,
                  current: i,
                  progress: (i + progress) / allowed.length,
                );
              },
              cancelToken: cancelToken,
              idempotencyKey: idempotencyKey,
            ...
            _logger.d('[ImageUpload] Uploaded ${i + 1}/${allowed.length}: $url');

            state = ImageUploadState.uploading(
              total: allowed.length,
              current: i + 1,
              progress: (i + 1) / allowed.length,
            );
          } catch (e) {
            if (e is DioException && e.type == DioExceptionType.cancel) {
              _logger.d('[ImageUpload] Cancelled: ${allowed[i].path}');
              state = const ImageUploadState.idle();
              rethrow;
            }
            _logger.e('[ImageUpload] Upload failed for ${allowed[i].path}', error: e);
            rethrow;
          } finally {
            _cancelTokens.remove(allowed[i].path);
            try {
        ...
        // Increment free plan daily counter
        if (user.plan == 'free' && uploadedUrls.isNotEmpty) {
          await _incrementFreeDailyUsed(uploadedUrls.length);
        }

        state = ImageUploadState.completed(uploadedUrls);
        _logger.i('[ImageUpload] Done: ${uploadedUrls.length} images uploaded');
        return uploadedUrls;
      } catch (e, stack) {
        _logger.e('[ImageUpload] Failed', error: e, stackTrace: stack);
...
void main() async {
  ...
  // Initialize adaptive performance tuning FIRST — before any heavy work
  // Detects device tier (low/mid/high) and sets image cache, frame hints, memory watcher
  await AdaptivePerformanceManager.initialize();
  ...
  runZonedGuarded(() async {
    try {
      ...
      if (sentryDsn.isNotEmpty && (Platform.isAndroid || Platform.isIOS)) {
        try {
      ...
      await Workmanager().registerPeriodicTask(
        'offline_sync',
        'sync_offline_messages',
        frequency: AdaptivePerformanceManager.backgroundSyncInterval,
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    switch (state) {
      ...
      case AppLifecycleState.inactive:
        ...
        AdaptivePerformanceManager.onBackground();
        break;
      case AppLifecycleState.detached:
      case AppLifecycleState.hidden:
        break;
